...
fd.add(Fermion(i1, v1).addArrow())
fd.add(Fermion(v1, i2).addArrow())
fd.add(Fermion(o1, v2).addArrow(0.25))
fd.add(Fermion(v2, o2).addArrow(0.8))
g1, = fd.add(Gluino(v1, v2).bend(1.5).set3D())
fd.add(Gaugino(v1, v2).bend(-1.5).set3D())
fd.add(Graviton(midpoint(v2,o2), midpoint(v2,o1)).bend(0.5).set3D())
g1.setStyles([pyx.color.rgb.red])

fd.add(Label("A silly SUSY / graviton demo", x=0, y=2.2))
