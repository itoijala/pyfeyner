...
out1 = Point(4, -2)
out2 = Point(4,  2)
in_vtx, = fd.add(Vertex(-2, 0, mark=CircleMark()))
out_vtx, = fd.add(Vertex(2, 0, mark=CircleMark()))

fd.add(Label("Drell-Yan QCD vertex correction", x=0, y=2))

fd.add(Fermion(in1, in_vtx).addArrow().addLabel(r"\Pelectron"))
fa2, = fd.add(Fermion(in_vtx, in2).addArrow().addLabel(r"\Ppositron"))
fa2.addParallelArrow(size=0.1, displace=-0.06, sense=-1)
fd.add(Photon(in_vtx, out_vtx).addLabel(r"\Pphoton/\PZ"))
fb1, = fd.add(Fermion(out1, out_vtx).addArrow(0.2).addLabel(r"\APquark"))
fb1.addParallelArrow(size=0.1, displace=-0.06, sense=-1)
fd.add(Fermion(out_vtx, out2).addArrow(0.8).addLabel(r"\Pquark"))
glu, = fd.add(Gluon(midpoint(out_vtx, out1), midpoint(out_vtx, out2)).set3D())
